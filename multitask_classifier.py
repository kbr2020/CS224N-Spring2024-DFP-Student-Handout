...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config,cosinus_m = False):
        ...
        self.linear_sst = torch.nn.Linear(config.hidden_size, N_SENTIMENT_CLASSES)
        self.dropout_sst = torch.nn.Dropout(config.hidden_dropout_prob)

        self.linear_par = torch.nn.Linear(config.hidden_size, 1)
        self.dropout_par = torch.nn.Dropout(config.hidden_dropout_prob)
        ...
        if (self.cosinus_m):
            ...
            self.Emb_sts2 = torch.nn.Linear(config.hidden_size, 128)
            self.fin_lin_sts = torch.nn.Linear(3*128,1)
    ...
    def forward_cos_emb(self, input_ids, attention_mask):
        ...
        output = self.Emb_dropout_sts2(self.act_f(output))
        output = self.Emb_sts2(output)
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        ...
        logits = self.linear_sst(self.dropout_sst(first_tk))
        return logits
    ...
    def predict_similarity(self,
        ...
        if (self.cosinus_m == False):
            ...
            # logit = (F.cosine_similarity(output_1,output_2)+1)/2
            Before_final = torch.cat((output_1,output_2,torch.abs(output_1 - output_2)),dim = 1)
            logit = F.relu(self.fin_lin_sts(Before_final))
        return logit
...
def train_multitask(args):
    ...
    para_dev_data = SentencePairDataset(para_dev_data, args, reduced = 1)
    ...
    for epoch in range(args.epochs):
        ...
        (sentiment_accuracy,sst_y_pred, sst_sent_ids, paraphrase_accuracy, para_y_pred, para_sent_ids,
...
def get_args():
    ...
    parser.add_argument("--fair_dev", type=str, default="data/gender_fair_train.csv")
